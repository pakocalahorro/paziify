...
updatedSessions.forEach((s, index) => {
    ...
    // Skip the generic headers in logic
...
renameTable.filter(s => s.isTechnical).forEach(s => {
    report += `| ${s.id} | ${s.title} | ${s.category} | \`${s.newName || s.new}\` |\n`;
});
...
let renameMd = '# Guía de Renombrado en Supabase\n\n| Archivo Original | Nuevo Nombre |\n|------------------|--------------|\n';
renameTable.forEach(row => {
    renameMd += `| ${row.old} | ${row.new} |\n`;
});
fs.writeFileSync(renameGuidePath, renameMd, 'utf8');

console.log('🚀 All files and reports successfully synchronized with CLEAN filenames.');
