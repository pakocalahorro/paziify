...
// The first 18 IDs are the technical ones
const technicalIds = [
...
// Re-parsing logic to generate a NEW report with the split
...
sessionBlocks.forEach((block, index) => {
    try {
        ...
        if (isTechnical) {
            ...
            newFileName = `tecnica_${num}_${sanitizedTitle}.mp3`;
        } else {
            // Pick a guide based on category for the new 101
            ...
            const sanitizedTitle = title.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-').trim('-');
            newFileName = `${category}_${assignedGuide.toLowerCase()}_${num}_${sanitizedTitle}.mp3`;
        }
...
console.log('# Informe de Reorganización: Técnicas vs Guiadas\n');
console.log('## Sesiones Técnicas (Core Paziify)');
console.log('| ID | Título | Sub-categoría | Nomenclatura |');
console.log('|----|--------|---------------|--------------|');
...
console.log(`\n**Resumen:** Técnicas (${audit.filter(s => s.isTechnical).length}) | Guiadas (${audit.filter(s => !s.isTechnical).length})`);
